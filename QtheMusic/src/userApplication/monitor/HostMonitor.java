...
public class HostMonitor implements ConnectionMonitor{
	...
	public synchronized boolean setID(int hostId) {
		if(hostId==ServerMonitor.NO_AVAILABLE_ID){
			...
			System.err.println("Invalid port number format message received from centralserver during setup");
			hostId=UNSET_HOST_ID;
		}else{
			this.hostId=hostId;
			statusMessage="No ID is currently available";
	...
	public synchronized void waitForServerConnectionToClose(Socket socket) throws InterruptedException, IOException {
	...
	public synchronized void setCentralServerAliveStatus(boolean serverConnectionIsAlive){
